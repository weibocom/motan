...
public class ConsulRegistry extends FailbackRegistry {
	...
	protected void doUnsubscribe(URL url, NotifyListener listener) {
		...
		HashMap<URL, NotifyListener> clusterListeners = subscribeListeners
				.get(ConsulUtils.getUrlClusterInfo(url));
		...
		if (res != null) {
			...
			if (services != null && !services.isEmpty()
				...
				for (ConsulService hservice : services) {
					try {
						URL url = ConsulUtils.buildUrl(hservice);
						String cluster = ConsulUtils.getUrlClusterInfo(url);
			...
			for (Entry<String, List<URL>> entry : groupUrls.entrySet()) {
				...
				if (groupMap != null) {
					...
					if (newUrls == null || newUrls.isEmpty()
							|| ConsulUtils.isSame(entry.getValue(), oldUrls)) {
						change = false; // 不需要更新
					} else {
		...
		public void run() {
			...
			synchronized (listeners) {
				for (Entry<URL, NotifyListener> entry : listeners.entrySet()) {
					ConsulRegistry.this.notify(entry.getKey(), entry.getValue(), urls);
				}
			}
		}
